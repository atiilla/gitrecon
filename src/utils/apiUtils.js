  ...
  try {
    ...
    if (url.includes("api.github.com") && response.headers) {
      ...
      if (remaining && limit && resetHeader) {
        ...
        console.log(
          ColorUtils.dim(
            `Rate limit: ${remaining}/${limit} (Resets: ${resetTime.toLocaleTimeString()})`
          )
        );
        ...
        if (parseInt(remaining) < 10) {
          console.warn(
            ColorUtils.yellow(
              `Warning: GitHub API rate limit is getting low (${remaining} remaining)`
            )
          );
        }
      }
    ...
    if (error.response) {
      ...
      if (
        ...
        console.error(
          ColorUtils.red(
            `Error: GitHub API rate limit exceeded. Resets at ${resetTime.toLocaleTimeString()}`
          )
        );
      } else {
        console.error(
      ...
      console.error(
        ColorUtils.red("Network Error: No response received from server")
      );
